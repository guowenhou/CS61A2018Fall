...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return max(1,(score // 10)*2-(score%10))
    # END PROBLEM 2


def take_turn(num_rolls, opponent_score, dice=six_sided):
    """Simulate a turn rolling NUM_ROLLS dice, which may be 0 (Free ).
    ...
    "*** YOUR CODE HERE ***"
    #if num_rolls ==0 :
    #    return max(free_bacon(opponent_score),1)
    #else :
    #    compute_score = roll_dice(num_rolls, dice)
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if abs((player_score % 100) // 10 - (player_score % 100) % 10) == abs((opponent_score%100) // 10 - (opponent_score%100) % 10 ): 
  
    # player_score % 100 先将其转为为两位数
    # //10分离出十位， %10 分离出个位
        return True
    else :
        return False
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while i < 11:
        if averaged_roll_dice(i,dice) > s:
            s = averaged_roll_dice(i,dice)
            result = i
        i +=1
    return result
