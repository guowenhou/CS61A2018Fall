...
def put_piece(board, max_rows, column, player):
    ...
    #k = 0
    max_rows = max_rows -1
    while (board[abs(max_rows)][0] == 'X' or board[abs(max_rows)][0] == 'O') and max_rows != -1:
        max_rows -=1
    if  max_rows == -1:
        return [max_rows,board]
    else :
        #col=[i[0] for i in board]
        #col=replace_elem(col, max_rows, player)
        
        #while k < len(col):
        #    if col[k] == player:
        #        board[k][0] = col[k]
        #   k+=1
        new_row = replace_elem(board[max_rows], column, player)
        new_board = replace_elem(board, max_rows, new_row)
    return [max_rows, board]
...
def print_board(board, max_rows, max_cols):
    ...
    for i in range(max_rows):
        ...
        for k in range(max_cols):
            result.append(get_piece(board,i,k))
        print(' '.join(result))
