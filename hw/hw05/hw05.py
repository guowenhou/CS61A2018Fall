...
def balanced(m):
    ...
    if length(left(m)) * total_weight(end(left(m))) == total_weight(end(right(m))) * length(right(m)) :
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,p):
        nonlocal err_pass 
        if len(err_pass) == 3 :
            return("Your account is locked. Attempts: %s"%(err_pass))
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(count,password ):
        if password in new_password: 
            return withdraw(count,old_password) 
        else :
            return withdraw(count, password)
            
    msg = withdraw(0, old_password)  
    if type(msg) == str:
        return msg
    else:
        return joint
